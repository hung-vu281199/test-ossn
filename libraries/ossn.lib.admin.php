...
function ossn_admin() {
        ...
        ossn_register_admin_sidemenu('admin:categories', 'admin:categories', ossn_site_url('administrator/categories'), ossn_print('admin:sidemenu:categories'));

        ossn_register_admin_sidemenu('admin:point', 'admin:point', ossn_site_url('administrator/point'), ossn_print('admin:sidemenu:point'));
		...
		if(ossn_isAdminLoggedin()) {
                ...
                ossn_register_action('admin/category/add', ossn_route()->actions . 'administrator/category/add.php');
                ossn_register_action('admin/category/edit', ossn_route()->actions . 'administrator/category/edit.php');
                ossn_register_action('admin/category/delete', ossn_route()->actions . 'administrator/category/delete.php');


            ossn_register_action('admin/point/add', ossn_route()->actions . 'administrator/point/add.php');
            ossn_register_action('admin/point/edit', ossn_route()->actions . 'administrator/point/edit.php');
            ossn_register_action('admin/point/delete', ossn_route()->actions . 'administrator/point/delete.php');
		}
...
function ossn_administrator_pagehandler($pages) {
		...
		switch($page) {
				...
				case 'edituser':
						...
						break;
                case 'add-category':
                        $title                = ossn_print('admin:add:category');
                        $contents['contents'] = ossn_plugin_view("pages/administrator/contents/category/add");
                        $contents['title']    = $title;
                        $content              = ossn_set_page_layout('administrator/administrator', $contents);
                        echo ossn_view_page($title, $content, 'administrator');
                        break;
                case 'categories':
                        $title                = ossn_print('admin:categories:list');
                        $contents['contents'] = ossn_plugin_view("pages/administrator/contents/categories/list");
                        $contents['title']    = $title;
                        $content              = ossn_set_page_layout('administrator/administrator', $contents);
                        echo ossn_view_page($title, $content, 'administrator');
                        break;
                case 'edit-category':
                        $category = array();
                        if(isset($pages[1])) {
                            $category['category'] = ossn_category_by_guid($pages[1]);
                        }
                        $title                = ossn_print('admin:edit:category');
                        $contents['contents'] = ossn_plugin_view("pages/administrator/contents/category/edit", $category);
                        $contents['title']    = $title;
                        $content              = ossn_set_page_layout('administrator/administrator', $contents);
                        echo ossn_view_page($title, $content, 'administrator');
                        break;
                case 'point':
                    $title                = ossn_print('admin.point:manager');
                    $contents['contents'] = ossn_plugin_view("pages/administrator/contents/point/list");
                    $contents['title']    = $title;
                    $content              = ossn_set_page_layout('administrator/administrator', $contents);
                    echo ossn_view_page($title, $content, 'administrator');
                    break;
                case 'add-point':
                    $title                = ossn_print('admin.add:point');
                    $contents['contents'] = ossn_plugin_view("pages/administrator/contents/point/add");
                    $contents['title']    = $title;
                    $content              = ossn_set_page_layout('administrator/administrator', $contents);
                    echo ossn_view_page($title, $content, 'administrator');
                    break;
                case 'edit-point':
                    $ps = array();
                    if(isset($pages[1])) {
                        $ps['point-setting'] = ossn_point_setting_by_guid($pages[1]);
                    }
                    $title                = ossn_print('admin.edit:point');
                    $contents['contents'] = ossn_plugin_view("pages/administrator/contents/point/edit", $ps);
                    $contents['title']    = $title;
                    $content              = ossn_set_page_layout('administrator/administrator', $contents);
                    echo ossn_view_page($title, $content, 'administrator');
                    break;
