...
function ossn_profile() {
		...
		if(ossn_isLoggedin()) {
            ...
            ossn_register_action('profile/kyc', __OSSN_PROFILE__ . 'actions/kyc.php');
...
function profile_edit_page($hook, $type, $return, $params) {
		...
		if($page == 'edit') {
				...
				if($section == 'kyc'){
                    ...
                    $params = array(
                        'action' => ossn_site_url() . 'action/profile/kyc',
                        'component' => 'OssnProfile',
                        'class' => 'ossn-kyc-form',
                        'params' => $user
                    );
                    $form   = ossn_view_form('kyc', $params, false);
                    $params['section']  = $section;

                    $args['user']    = $user['user'];
                    $args['section'] = $section;
                    $params['contents'] = ossn_call_hook('profile', 'kyc:section', $args, $form);
                    echo ossn_plugin_view('profile/kyc', $params);
